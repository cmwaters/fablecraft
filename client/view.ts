...
export class View {
		...
		if (this.movementInterval === null) {
			...
			this.movementInterval = setInterval(() => {
				...
				if (Math.abs(diff.x) < 5 && Math.abs(diff.y) < 5) {
					this.shift(diff);
					console.log(
						"final pos: " + g.string(this.cards[depth][cardIdx].centerPos())
					);
	...
	calculateCardWidth(): number {
		return Math.min(
			Math.max(0.5 * this.element.clientWidth, Config.card.width.min),
			Config.card.width.max
		);
	}
	...
	createAbove(): void {
		...
		let newCard = new Card(
			this,
			g.copy(this.currentCard.pos()),
			this.calculateCardWidth()
		);
		this.cards[this.currentDepth].splice(this.currentIndex, 0, newCard);
		this.slideBottom(
			newCard.height() + this.margin.height + Config.card.toolbarHeight
		);
		if (pIdx !== null) {
			...
			if (
				...
				this.cards[this.currentDepth - 1][
					pIdx
				].firstChildIdx = this.currentIndex;
			}
	...
	branch(): void {
        ...
        console.log("branch 3");
				...
				console.log("pos: " + pos.x);
				this.cards[this.currentDepth + 1].splice(
					idx,
					0,
					new Card(this, pos, this.cardWidth)
				);
	...
	private childInsertionIndex(): number {
		for (let idx = this.currentIndex - 1; idx >= 0; idx--) {
			if (this.cards[this.currentDepth][idx].firstChildIdx !== null) {
				return (
					this.cards[this.currentDepth][idx].firstChildIdx +
					this.cards[this.currentDepth][idx].childrenCount
				);
			}
		}
		return 0;
	...
	deleteCard(): void {
		...
		if (pIdx !== null) {
			...
			if (this.cards[this.currentDepth - 1][pIdx].firstChildIdx === pIdx) {
		...
		// We are deleting the last card in this column. We must focus back to the parent
		if (this.cards[this.currentDepth].length === 0) {
			if (pIdx === null) {
				alert(
					"panic: current depth is not the root depth but the card has no parent index"
				);
			}
			this.currentDepth--;
			...
			this.cards[this.currentDepth].length === 1 ||
			this.currentIndex < 0
		) {
	...
	slideBottom(delta: number): void {
		...
		for (
			...
			i < this.cards[this.currentDepth].length;
			i++
		) {
